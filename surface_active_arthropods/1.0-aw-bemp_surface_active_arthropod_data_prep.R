...
artho_raw <- read_csv("data/raw/qaqc_pitfalls_1997-2019-RawData.csv",
                      na=c('NA','.','#VALUE!'))
artho_raw

# Check the trap numbers. Should only be 1 - 20. 
unique(artho_raw$Trap)

# What years are in the data set
...
write.table(artho_line_numbers, "data/interim/bemp_surface_active_arthropod_1997_2019_w_line_numbers.csv",
...
artho_annual_mean_lines <- artho_line_numbers %>% group_by(`Site number`, Year, lettercode, numbercode, family,
...
artho_annual_mean_counts_species <- artho_annual_mean_lines %>% 
  group_by(`Site number`, Year, lettercode, numbercode, family, class, order, genspp) %>%
