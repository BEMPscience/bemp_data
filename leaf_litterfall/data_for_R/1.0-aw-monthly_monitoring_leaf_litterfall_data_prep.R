...
litterfall$date_collected <- as.Date( paste( litterfall$month, litterfall$day,
                                       litterfall$year,
                                       sep = "." ), format = "%m.%d.%Y" )
# Transform data to g/m^2
tub_area_function <- function(x) {round((x/0.13),2)} # liiterfall tubs are 0.13 square meters.
litterfall <- mutate_at(litterfall, vars(cw:wood),
...
monthly_litterfall_means <- litterfall_site_info %>% group_by(date_collected, site, site_name, site_lat, site_long, 
                                                month, year) %>% 
...
# Time series plot of select BEMP sites
monthly_litterfall_means_bosque_farms <- monthly_litterfall_means %>% 
  filter(site == 27)

monthly_litterfall_means_bosque_farms

monthly_litterfall_means_bosque_farms %>% filter(year > 2014 & year <2018) %>% 
  ggplot(aes(x=date_collected, y=cw)) + geom_line() + geom_point() +
  xlab("Date Collected") + ylab("Cottonwood leaf litterfall (g/m^2)")
